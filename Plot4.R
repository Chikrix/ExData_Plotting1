...
names(HD2)
names(HD2) <- gsub(" ",".",names(HD2)) # I gave the column names an appropriate name
names(HD2)
HD2$Date
table(HD2$Date)
...
with(Ddays,plot(DT,as.vector(Voltage),type="l",ylab="Voltage",xlab="datetime"))
...
par(col="black")
with(Ddays,plot(DT,as.vector(Global.reactive.power),type="l",xlab="datetime",ylab="Global.reactive.power"))
dev.off()